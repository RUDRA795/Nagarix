...
async function runTests() {
  ...
  try {
    const cityStatus = await (civicTools.get_city_status as any).execute({}, {} as any);
  ...
  try {
    const wardStats = await (civicTools.get_ward_statistics as any).execute({ wardNumber: 12 }, {} as any);
  ...
  // Test search_issues
  try {
    const searchRes = await (civicTools.search_issues as any).execute({ category: 'Road', limit: 3 }, {} as any);
  ...
  try {
    const priorityRes = await (civicTools.get_priority_recommendations as any).execute({}, {} as any);
